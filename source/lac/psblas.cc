...
namespace PSCToolkit
{
  ...
  namespace Communicator
  {
     ...
     * Utilities::MPI::MPI_InitFinalize class. Observe that if the communicator
     * was initialized with Init(), then the responsability of exiting the MPI
     * communicator lies on the user and on a call on
     * PSCToolkit::Communicator::Exit() routine.
     */
    psb_c_ctxt *
    InitFromMPI(MPI_Comm comm)
    {
     ...
     */
    void
    Info(psb_c_ctxt *cctxt, int *iam, int *nproc)
    {
     ...
     */
    psb_c_descriptor *
    CreateDescriptor(IndexSet &index_set, psb_c_ctxt cctxt)
    {
     ...
     */
    int
    DescriptorAssembly(psb_c_descriptor *cd)
    {
      // Assemble the descriptor
      int result = psb_c_cdasb(cd);
      if (result != 0)
        {
          deallog << "Error assembling PSBLAS descriptor: " << result
                  << std::endl;
        }
      return result;
    }
      ...
      if (cd != nullptr)
        {
          ...
          if (result != 0)
            {
              deallog << "Error freeing PSBLAS descriptor: " << result
                      << std::endl;
            }
        ...
        {
          deallog << "Warning: Attempted to free a null PSBLAS descriptor."
                  << std::endl;
          return -1; // Failure
        }
    }
  ...
  PSCToolkit::SparseMatrix::set(
    const PSCToolkit::SparseMatrix::size_type  i,
    const PSCToolkit::SparseMatrix::size_type  j,
    const PSCToolkit::SparseMatrix::value_type value)
  {
  ...
  void
  PSCToolkit::SparseMatrix::add(
    const PSCToolkit::SparseMatrix::size_type               row,
    const PSCToolkit::SparseMatrix::size_type               ncols,
    const std::vector<PSCToolkit::SparseMatrix::size_type> &col_indices,
    const PSCToolkit::SparseMatrix::value_type             *values,
    const bool,
    const bool)
  {
  ...
  PSCToolkit::SparseMatrix::add(
    const PSCToolkit::SparseMatrix::size_type   row,
    const PSCToolkit::SparseMatrix::size_type   ncols,
    const PSCToolkit::SparseMatrix::size_type  *col_indices,
    const PSCToolkit::SparseMatrix::value_type *values,
    const bool,
    const bool)
  {
    ...
    for (PSCToolkit::SparseMatrix::size_type i = 0; i < ncols; ++i)
      {
        irw[i] = row;
        icl[i] = col_indices[i];
      }
  ...
  {
    /**
     * Create a new PSBLAS sparse matrix and initialize it with the given
     * descriptor.
     *
     ...
     */
    psb_c_dspmat *
    CreateSparseMatrix(psb_c_descriptor *cd)
    {
      // Create a new PSBLAS sparse matrix
      ...
      if (info != 0)
        {
          deallog << "Error initializing PSBLAS sparse matrix: " << info
                  << std::endl;
        }
      return matrix;
    }
                ...
                psb_c_descriptor *cdh)
    {
      ...
      if (info != 0)
        {
          deallog << "Error inserting value into PSBLAS sparse matrix: " << info
                  << std::endl;
        }
      return info;
    }
      ...
      psb_c_descriptor                           *cdh)
    {
      ...
      if (info != 0)
        {
          deallog << "Error inserting values into PSBLAS sparse matrix: "
                  << info << std::endl;
        }
      ...
      if (info != 0)
        {
          deallog << "Error inserting values into PSBLAS vector: " << info
                  << std::endl;
        }
     ...
     */
    int
    AssembleSparseMatrix(psb_c_dspmat *mh, psb_c_descriptor *cdh)
    {
      int info = -1;
      // Check if the sparse matrix is not already assembled
      if (!psb_c_dis_matasb(mh, cdh))
        {
          ...
          if (info != 0)
            {
              deallog << "Error assembling PSBLAS sparse matrix: " << info
                      << std::endl;
            }
        }
      return info;
     ...
     */
    int
    FreeSparseMatrix(psb_c_dspmat *mh, psb_c_descriptor *cdh)
    {
      if (mh != nullptr && cdh != nullptr)
        ...
        {
          deallog << "Warning: Attempted to free a null PSBLAS sparse matrix."
                  << std::endl;
          return -1; // Failure
        }
    }
  ...
  namespace PSBVector
  {
    ...
    /**
     * Create a new PSBLAS vector and initialize it with the given
     * descriptor.
     *
     ...
     */
    psb_c_dvector *
    CreateVector(psb_c_descriptor *cd)
    {
     ...
     */
    int
    AssembleVector(psb_c_dvector *vector, psb_c_descriptor *cd)
    {
      if (vector == nullptr || cd == nullptr)
        {
          deallog << "Error assembling PSBLAS vector: null pointer."
                  << std::endl;
          return -1;
        }
     ...
     */
    int
    FreeVector(psb_c_dvector *vector, psb_c_descriptor *cd)
    {
      if (vector != nullptr && cd != nullptr)
        {
        ...
        {
          deallog << "Warning: Attempted to free a null PSBLAS vector."
                  << std::endl;
          return -1; // Failure
        }
    }
  ...
  namespace Solvers
  {
     ...
     */
    psb_c_dprec *
    CreateBasePreconditioner(psb_c_ctxt cctxt, const char *ptype)
    {
      psb_c_dprec *ph = psb_c_new_dprec();
      if (ph == nullptr)
        {
          deallog << "Error allocating memory for PSBLAS preconditioner."
                  << std::endl;
          return nullptr;
        }
      ...
      if (info != 0)
        {
          deallog << "Error initializing PSBLAS preconditioner: " << info
                  << std::endl;
          free(ph);
          return nullptr;
        }
     ...
     */
    int
    BasePrecBuild(psb_c_dprec *ph, psb_c_dspmat *mh, psb_c_descriptor *cdh)
    {
      if (ph == nullptr || mh == nullptr || cdh == nullptr)
        {
          deallog << "Error building PSBLAS preconditioner: null pointer."
                  << std::endl;
          return -1; // Failure
        }
      ...
      if (info != 0)
        {
          deallog << "Error building PSBLAS preconditioner: " << info
                  << std::endl;
        }
     ...
     */
    int
    FreeBasePreconditioner(psb_c_dprec *ph)
    {
      if (ph != nullptr)
        {
          ...
          if (result != 0)
            {
              deallog << "Error freeing PSBLAS preconditioner: " << result
                      << std::endl;
            }
          return result; // Success
        }
        ...
        {
          deallog << "Warning: Attempted to free a null PSBLAS preconditioner."
                  << std::endl;
          return -1; // Failure
        }
    }
    ...
    /**
     * Create and initialize a PSBLAS solver options structure with
     * default values.
     *
    ...
    CreateSolverOptions()
    {
      psb_c_SolverOptions *opt =
        (psb_c_SolverOptions *)malloc(sizeof(psb_c_SolverOptions));
      if (opt == nullptr)
        {
          deallog << "Error allocating memory for PSBLAS solver options."
                  << std::endl;
          return nullptr;
        }
     ...
     */
    void
    SetSolverOptions(psb_c_SolverOptions *opt,
                     int                  itmax,
                     int                  itrace,
                     int                  irst,
                     int                  istop,
                     double               eps)
    {
      if (opt == nullptr)
        {
          deallog << "Error: null pointer passed to SetSolverOptions."
                  << std::endl;
          return;
        }
     ...
     */
    void
    PrintSolverOptions(const psb_c_SolverOptions *opt, psb_c_ctxt *cctxt)
    {
      if (opt == nullptr)
        {
     ...
     */
    void
    PrintSolverOptions(const psb_c_SolverOptions *opt,
                       psb_c_ctxt                *cctxt,
     ...
     */
    void
    FreeSolverOptions(psb_c_SolverOptions *opt)
    {
      if (opt != nullptr)
        {
          free(opt);
        ...
        {
          deallog
            << "Warning: Attempted to free a null PSBLAS solver options structure."
            << std::endl;
        }
    }
    ...
    BaseKrylov(const char          *method,
               ...
               psb_c_SolverOptions *opt)
    {
      if (method == nullptr || ah == nullptr || bh == nullptr ||
          xh == nullptr || cdh == nullptr || opt == nullptr)
        {
          deallog << "Error: null pointer passed to Krylov solver."
                  << std::endl;
          return -1; // Failure
        }
      ...
      if (info != 0)
        {
          deallog << "Error solving linear system with PSBLAS Krylov solver: "
                  << info << std::endl;
        }
      return info; // Success or failure
    }
