...
namespace PSCToolkit
{
  ...
  Vector::mean_value() const
  {
    ...
    return Utilities::MPI::sum(local_sum_of_values, communicator) /
           locally_owned_size();
  }
  ...
  Vector::add(const value_type s)
  {
    ...
    Assert(!has_ghost_elements(), ExcGhostsPresent());
    int err = psb_c_dgeaxpby(
      1.0, psblas_vector, s, psblas_vector, psblas_descriptor.get());
    AssertThrow(err == 0, internal::ExcAXPBY(err));
  }
  ...
  Vector::compress(const VectorOperation::values operation)
  {
    Assert(has_ghost_elements() == false,
           ExcMessage("Calling compress() is only useful if a vector "
                      ...
                      "vectors."));
    // AssertThrow(operation == VectorOperation::values::add,
    // ExcNotImplemented()); We check the state of the vector...
