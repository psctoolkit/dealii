...
namespace FETools
{
  ...
  namespace internal
  {
    ...
    back_interpolate(
      const DoFHandler<dim, spacedim> &dof1,
      const AffineConstraints<PSCToolkitWrappers::Vector::value_type>
                                       &constraints1,
      const PSCToolkitWrappers::Vector &u1,
      const DoFHandler<dim, spacedim>  &dof2,
      const AffineConstraints<PSCToolkitWrappers::Vector::value_type>
                                 &constraints2,
      PSCToolkitWrappers::Vector &u1_interpolated)
    {
      // if u1 is a parallel distributed PSBLAS vector, we create a
      // vector u2 with based on the sets of locally owned and relevant
      // dofs of dof2
      const IndexSet &dof2_locally_owned_dofs = dof2.locally_owned_dofs();
      const IndexSet  dof2_locally_relevant_dofs =
      ...
      PSCToolkitWrappers::Vector u2_out(dof2_locally_owned_dofs,
                                        u1.get_mpi_communicator());
      interpolate(dof1, u1, dof2, constraints2, u2_out);
      PSCToolkitWrappers::Vector u2(dof2_locally_owned_dofs,
                                    dof2_locally_relevant_dofs,
                                    u1.get_mpi_communicator());
      u2 = u2_out;
      interpolate(dof2, u2, dof1, constraints1, u1_interpolated);
