...
// This test checks the assembly of a PSBLAS sparse matrix when the
// SparsityPattern is set using PSBLAS.
...
main(int argc, char **argv)
{
  ...
  PSCToolkit::SparsityPattern sparsity_pattern(locally_owned_dofs,
                                               mpi_communicator);
  PSCToolkit::SparseMatrix    psblas_matrix(sparsity_pattern, mpi_communicator);
  ...
  deallog << "Process " << Utilities::MPI::this_mpi_process(mpi_communicator)
          << " of " << Utilities::MPI::n_mpi_processes(mpi_communicator)
